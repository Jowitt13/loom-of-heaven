/**
 * reading-lint — deterministic, offline "term firewall" for the FINAL user-visible
 * Channel B topic report (ADR 0011). It does NOT read facts/schema; it inspects only
 * the produced markdown text: sections 1-5 (+ the trailing follow-up line) are a
 * no-term zone, while 专业依据 / 信息可靠性与声明 are exempt. Tiered severity: 命理 terms
 * and strong consultant-jargon are hard errors; soft jargon is a warning except in the
 * 30秒看懂 / first-200-char strict zone (or `simple` mode), where all jargon is an error.
 * It flags problems and gives a replacement hint; the host rewrites (no mechanical
 * replacement). Static text structure only — it cannot prove a host model complies
 * 100% of the time (see docs/VALIDATION.md).
 */
...
export type ViolationCategory = '命理' | '黑话强' | '黑话软' | '空话' | '重复' | '越界';
...
// Context-bound patterns (avoid false positives on common words like 上升/下降/亮度字).
const CUSTOM_MINGLI: RegExp[] = [
  /[甲乙丙丁戊己庚辛壬癸][子丑寅卯辰巳午未申酉戌亥]/g, // 干支组合 (甲戌 / 戊申 …)
  /上升(?:点|星座|宫|[（(]?\s*ASC)/g,
  /下降(?:点|星座|宫|[（(]?\s*DSC)/g,
  /中天|\bMC\b/g,
  /亮度\s*[“”"『「]?\s*[庙旺得利平不陷]/g,
...
function buildTermSpecs(): TermSpec[] {
  ...
  for (const t of [...MINGLI_BAZI, ...MINGLI_ZIWEI, ...MINGLI_WEST, ...MINGLI_TIME]) {
    specs.push({ re: new RegExp(esc(t), 'g'), category: '命理' });
  }
...
const TERM_SPECS = buildTermSpecs();

const FOLLOWUP_RE = /还想看|想看更多|想继续看|继续(?:看|了解)|想深入看|接下来想看/;
// In the follow-up menu these double as legitimate TOPIC labels, not term leaks.
const FOLLOWUP_TOPIC_OK = new Set(['流年', '大运']);
...
/** Break the report into per-sentence units for the checked (non-exempt) zones. */
 ...
 * For `channel:'topic'` (Channel B), 命理 terms + strong jargon are errors in sections
 * 1-5 and the follow-up; soft jargon is a warning there but an error in the 30秒看懂 /
 * first-200-char zone or when `simple` is set. 专业依据 / 信息可靠性与声明 are exempt.
 */
