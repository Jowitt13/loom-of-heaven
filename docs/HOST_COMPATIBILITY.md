...
> 当前分发状态：安装包来自 GitHub Release `v0.3.6`；Qoder、WorkBuddy 与豆包电脑版已有公开 ZIP。可下载性以 `install-manifest.json` 的 `published` 字段及 SHA-256 为准。
>
...
| 真机验证                | L3             | L3                                           | L3                          | L3                          |
| 当前分发来源            | 仓库（已公开） | GitHub Release `v0.3.6` ZIP                  | GitHub Release `v0.3.6` ZIP | GitHub Release `v0.3.6` ZIP |
...
| Qoder     | GitHub Release `v0.3.6` ZIP | full     | release-asset |
| WorkBuddy | GitHub Release `v0.3.6` ZIP | full     | release-asset |
| 豆包      | GitHub Release `v0.3.6` ZIP | full     | release-asset |

- 根清单当前为 `status: "published"`：安装包来自 GitHub Release `v0.3.6`，包含不可变下载地址与 SHA-256。安装器必须先检查所选平台的 `published`；为 `false` 时不读取 URL/hash、不下载、不安装。
...
- 更新迁移：只有线上清单将所选平台标记为已发布时，Agent 才可下载不可变 tag 资产、校验 SHA-256 并用 `migrate` 原子替换旧包。当前更新源是 v0.3.6，不能把本地旧包或缓存称作最新版。
