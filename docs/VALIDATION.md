...
source-traceable Ôø?not that divination predicts anything (handoff ¬ß9).
...
These items appear in the QODER_HANDOFF ¬ß9.1 long-term minimum bar but have no runnable
...
- Broader ESLint ruleset (style / type-aware rules) Ôø?only the import-boundary gate
...
The test count below comes from one real `pnpm run test` run Ôø?the single source of truth shared with
...
- Typecheck: clean. Tests: **472 tests / 29 files Ôø?all passing**. The Western provider
  (astronomy-engine, VSOP87 + NOVAS) passes the ADR-0003 Ôø?Ôø?gate two ways: wrapper-consistency
  (vs astronomy-engine's own output) plus an **independent JPL Horizons golden** (10 bodies √ó 3
  technical epochs fetched from the NASA/JPL Horizons service, query recorded in
  `packages/western/goldens/jpl-horizons.json`; worst deviation 0.20Ôø?; the sidereal zodiac (Lahiri), true node
  and asteroids have a dedicated **approximate** regression (continuity / sign-plausibility, not the
  Ôø?Ôø?gate); angles/houses are validated against the MC=RAMC and eastern-horizon oracles, the Zi Wei
  dynamic chart (ËøêÈôêÔø? is regression-anchored, the sourced BaZi interpretation rules (incl.
  ÂàëÂÜ≤ÂêàÂÆ≥/Á•ûÁÖû/Â§ßËøêÂêâÂá∂ `polarity`) are covered, and the cross-system interpretation-facts layer is
  ...
  Êó†ÊúØËØ≠Âå∫ term firewall; offline, no LLM Ôø?it proves the spec/sample structure, **not** that a host
  model follows the style 100% of the time). Docs-consistency `validate:docs` passes (four full hosts /
  render disabled / no wrong-ephemeris attribution / dev Node Ôø?24 vs run Node Ôø?22, with positive+negative
  ...
  ÂëΩÁêÜ/ÈªëËØù terms in sections 1-5 Ôø?still a static text gate, not a guarantee of host-model wording. Round 9 (ADR 0012) added an Á©∫ËØù (vagueness) check: abstract judgements in sections 1-5 must carry a concrete
  action/scene/observable/result nearby, and three REAL reports (male, 1990-06-15 14:20, Á§∫‰æãÂüéÂ∏Ç; ‰∫ã‰∏ö/ÊÑüÊÉÖ/
  Ë¥¢Ëøê, saved unpolished to `docs/round9-acceptance/`) lint to 0 error (test #8 reads them). The detector is a
  ...
  content check Ôø?malformed, over-limit or wrong-version input yields a stable not-ok result, never a
  ...
  and paragraph** in one shared pipeline (heading/text fields are plain text ‚Äî HTML/entity/Markdown
  ...
  per-item disclaimer coverage Ôø?EVERY plan disclaimer must be referenced (error severity, explicit
  ...
  parse+validation stages only Ôø?the CLI additionally caps the input file size before reading. It is
  ...
  smoke: **10/10**. Clean-dir forward test: **41/41** Ôø?8 realistic requests (incl. a horoscope, an
  interpret and a multi-person ÂêàÂ©ö synastry) across the CLI, and that `render` is disabled (exit 3,
...
## Boundary fixtures (36; Ôø?0 required for Phase 1)

`packages/test-fixtures` Ôø?each fixture records why its expectation is trustworthy (documented
IANA transition dates, standard offsets, or plain wall‚ÜîUTC arithmetic), never an engine snapshot.
...
winter UTC+8); different longitudes Ôø?different mean solar time; near day/zi-hour boundaries;
unknown & approximate time; out-of-range years; unknown timezone; lunar-not-yet.

Property tests: wall‚ÜîUTC round-trip invariant; equation-of-time bounds; apparent = mean + EoT;
...
- Western: at least one set cross-checked against JPL/Swiss; main-body positions within Ôø?
...
- Minimum targets: time/location 30 (met: 36), Western 20 (met: 30 Ôø?JPL Horizons golden
