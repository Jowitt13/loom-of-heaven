...
source-traceable ‚Ä?not that divination predicts anything (handoff ¬ß9).
...
- Broader ESLint ruleset (style / type-aware rules) ‚Ä?only the import-boundary gate
...
The test count below comes from one real `pnpm run test` run ‚Ä?the single source of truth shared with
...
- Typecheck: clean. Tests: **385 tests / 27 files ‚Ä?all passing**. The Western provider
  (astronomy-engine, VSOP87 + NOVAS) passes the ADR-0003 ‚â?‚Ä?gate two ways: wrapper-consistency
  ...
  `packages/western/goldens/jpl-horizons.json`; worst deviation 0.20‚Ä?; the sidereal zodiac (Lahiri), true node
  and asteroids have a dedicated **approximate** regression (continuity / sign-plausibility, not the
  ‚â?‚Ä?gate); angles/houses are validated against the MC=RAMC and eastern-horizon oracles, the Zi Wei
  dynamic chart (ËøêÈôêÁõ? is regression-anchored, the sourced BaZi interpretation rules (incl.
  ...
  Êó†ÊúØËØ≠Âå∫ term firewall; offline, no LLM ‚Ä?it proves the spec/sample structure, **not** that a host
  ...
  ÂëΩÁêÜ/ÈªëËØù terms in sections 1-5 ‚Ä?still a static text gate, not a guarantee of host-model wording. Round 9 (ADR 0012) added an Á©∫ËØù (vagueness) check: abstract judgements in sections 1-5 must carry a concrete
  ...
  content check ‚Ä?malformed, over-limit or wrong-version input yields a stable not-ok result, never a
  ...
  per-item disclaimer coverage ‚Ä?EVERY plan disclaimer must be referenced (error severity, explicit
  ...
  parse+validation stages only ‚Ä?the CLI additionally caps the input file size before reading. It is
  ...
  smoke: **10/10**. Clean-dir forward test: **41/41** ‚Ä?8 realistic requests (incl. a horoscope, an
...
`packages/test-fixtures` ‚Ä?each fixture records why its expectation is trustworthy (documented
...
winter UTC+8); different longitudes ‚Ü?different mean solar time; near day/zi-hour boundaries;
...
- Minimum targets: time/location 30 (met: 36), Western 20 (met: 30 ‚Ä?JPL Horizons golden
