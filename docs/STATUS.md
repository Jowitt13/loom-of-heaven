...
| `pnpm run test`                | 327 tests / 27 files — all passing (all systems + JPL Horizons 独立 golden + interpret + 吉凶 + 合婚 + reading-lint/空话/重复/越界 + validate-answer 结构与措辞门禁（事实边界+全 section 安全扫描+资源上限，非语义正确性证明） + western-rules/ziwei-rules 语义规则 + 版本迁移/回滚/目标白名单 + PII 隐私护栏 green) |
