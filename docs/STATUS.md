...
- pnpm monorepo scaffolded: `packages/{contracts,time-location,orchestrator,test-fixtures}`,
  `tools/`, `skills/calculate-birth-charts/`, `docs/`.
- Dependency versions/licenses verified against the live npm registry (see `docs/LICENSE_AUDIT.md`).
- ADRs 0001–0004 (skill-first, TZDB, providers, toolchain). Docs: PRODUCT_SPEC, ARCHITECTURE,
...
- 36 sourced boundary fixtures (≥30 required) + unit/property tests.
...
- Western provider evaluated and **rejected at the ADR 0003 gate**: celestine 0.2.1 agrees with
  the wrapper-consistency cross-check (astronomy-engine, VSOP87 + NOVAS) to ≤1′ for Sun–Neptune but deviates up
...
### Phase W2 — Zi Wei dynamic charts (运限盘)

- Zi Wei dynamic charts via iztro's `horoscope()`: **大限/小限/流年/流月/流日/流时** for any target
  solar date, each with its re-placed twelve palaces, 运限四化 and (流年) 将前/岁前十二星.
...
### Phase W3 — sourced BaZi interpretation rules (旺衰/格局/喜用神/十神)
...
- Rules: 旺衰 strength (得令/得地/得势, 《子平真诠》), 格局 pattern from 月令本气 ten-god
  (《子平真诠》), 喜用神 direction via 扶抑 (《滴天髓》), 十神象义 (《渊海子平》). Versioned ruleset
...
- Verified on synthetic + real charts: 甲木卯月 → 偏强 with 建禄/月劫 honestly flagged; 正官格
  matched; 身弱 → 喜印比劫; the chart's ten-gods listed. Deterministic.
...
- **取格修正 + 应期 + 解读严谨 (ADR 0006):** 建禄仅禄位/阳刃仅刃位；杂气月透干取格（戊土辰月→杂气
  正财格，非建禄）；新增五行缺失、天干五合(日主合财)、大运/流年冲合应期(如 2028 申冲寅)；
  reading-style/SKILL 增反绝对化与术语/系统隔离/一致性铁律。
- **逐年流年 + 语义精修 (ADR 0007):** 引擎按当前年锚定逐年产出流年主题(天干/地支十神+合冲)；日主
  多重合合并为一条(贴身/遥见、不双合化)；缺 X 改述为“需后天训练、非无能力”；概率话术改为趋势；
  reading-style/SKILL 增十神象义/财格/官杀藏≠排斥组织/紫微忌单星/水逆≠慢/校对敏感项。
- **常见追问 + 多人合婚 (ADR 0008):** 新增婚姻/正缘应期、适合行业、配偶画像事实 + 追问 playbook；新增
  `@ming/synastry` 包与 `synastry` 命令（1-5 人、八字/紫微/占星三系、>2 人需 analyzePair）；SKILL
  多人工作流（先确认关系与分析哪两人）；反绝对化——不作“注定/必分”。W5 “synastry 不做”假设作废。

## Commands & results (2026-07-22)

The counts below are the output of one real `pnpm run verify:all` run — the single source of truth
...
| `pnpm run test`                | 260 tests / 22 files — all passing (all systems + interpret + 吉凶 + 合婚 + reading-lint/空话/重复/越界 + 版本迁移/回滚/目标白名单 + PII 隐私护栏 green) |
...
| `pnpm run verify:install`      | stable root = v0.1.3 published (release live), per-zip sha256 matches; candidate build unpublished/reproducible                                          |
...
| `pnpm run verify:all`          | green end to end                                                                                                                                         |
...
- Dependency **vulnerability** scan (`scan:deps`, `pnpm audit --prod`) and **secret** scan
  (`scan:secrets`) are now wired into `verify:all`. A dependency **license** scan, SPDX SBOM and a
...
## Next step (expansion roadmap complete → Phase 4 acceptance / Phase 5–6)

The approved expansion roadmap is now **complete**: Western natal charts (astronomy-engine, VSOP87 + NOVAS),
Zi Wei dynamic charts (运限盘), a sourced BaZi interpretation-rules package, and the cross-system
...
import-boundary gate are already wired into `verify:all`).
