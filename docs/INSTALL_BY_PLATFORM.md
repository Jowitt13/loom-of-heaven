...
> 当前分发状态：安装包来自 GitHub Release `v0.2.0`。Qoder、WorkBuddy 与豆包电脑版已有公开 ZIP；以 `install-manifest.json` 的 `published` 字段及 SHA-256 为准。
>
...
| 平台       | 用户需要做什么                          | 最终能力 |
| ---------- | --------------------------------------- | -------- |
| Qoder      | 发一句安装链接；Agent 下载并校验 v0.2.0 | 完整排盘 |
| WorkBuddy  | 发一句安装链接；Agent 下载并校验 v0.2.0 | 完整排盘 |
| 豆包电脑版 | 发一句安装链接；Agent 下载并校验 v0.2.0 | 完整排盘 |
| Codex      | 公开仓库可用                            | 完整排盘 |
...
> 当前公开安装源是 GitHub Release `v0.2.0`。候选 ZIP 仍只用于本地验证，不能作为公开“一句话安装”的来源；后续版本必须创建新的不可变 Release、完成重下验收后再更新清单。
...
安装包来自 GitHub Release `v0.2.0`。把顶部安装入口发给 Qoder 后，Agent 会先读取清单，确认 Qoder `published: true` 后下载不可变资产并校验 SHA-256；不要请求旧链接，也不要用本地缓存冒充最新版。
...
安装包来自 GitHub Release `v0.2.0`。安装入口会先读取清单，确认 WorkBuddy `published: true` 后才读取 URL/hash、下载并按宿主流程导入。
...
安装包来自 GitHub Release `v0.2.0`。安装入口会先读取清单，确认豆包 `published: true` 后才读取 URL/hash、下载并按宿主流程导入。
