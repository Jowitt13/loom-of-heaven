...
function readingLiteSkillMd(): string {
  const fm = [
    ...
    'description: >-',
    '  解读辅助版(reading-lite)——接收已由完整 Loom of Heaven 生成的命盘 facts(西占/八字/紫微/吠陀),',
    '  按自然叙述规范生成事业、感情、财运、学业解读（术语、规则机制与现实含义自然连写）。',
    '  **此版本不包含排盘引擎,不能自行计算西占、八字、紫微或吠陀位置。**',
  ...
  const body = [
    ...
    '3. 按 Loom of Heaven 的自然叙述规范生成事业、感情、财运等解读',
    '4. 仅在用户明确追问时提供技术来源、盘面细节或计算设置',
    '',
    '## 此版本不能做什么',
    '',
    ...
    '- 如果用户提供了 facts,按 `references/reading-style.md` 的自然叙述规范生成解读。',
    '- 专业术语可出现在正文中，但必须紧接着说明其规则机制与现实含义。',
    '- 默认输出只含自然段落；不展示告警标题、事实编号、规则路径、固定声明或自动追问。',
    '- 不给出确定性医疗、法律、投资、生死建议。',
    '',
    '## 工作流',
    '',
    ...
    '3. 先在内部把 facts/evidence/reason/caveat 组成“事实 → 规则机制 → 现实含义 → 条件”链路，再写成连续自然段。',
    '4. 术语、限制和建议只在它们与当前问题有关时自然融入正文。',
    '5. 默认不添加章节标题、专业依据、引擎警告、声明或追问菜单。',
    '',
    '### 用户请求排盘(无 facts)',
    '',
    '1. 告知用户此版本不能排盘',
    ...
    '## 参考文件',
    '',
    '- `references/reading-style.md` — 自然叙述规范（交付面、来源追踪、空话与越界检查）',
    ...
    '',
    '完全离线,不上传出生信息。涉及医疗、法律、投资或重大人生决定时，不给确定性判断。',
  ];
  return fm.join('\n') + '\n' + body.join('\n') + '\n';
}
